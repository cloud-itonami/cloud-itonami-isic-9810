(ns hhproductionops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (HouseholdProductionAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the HouseholdProductionGovernor
  ...
  human operator/programme coordinator. The approver resumes with
...
(defn build
  ...
  `hhproductionops.store/Store`).
  opts:
    :advisor      -- a `hhproductionops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HouseholdProductionAdvisor inference (the contained
      ...
      ;; HouseholdProductionGovernor -- independent censor (separate
